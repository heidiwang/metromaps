...
// Submit the form both on enter and on the button press
if (document.getElementById('filenameForm').attachEvent) {
	document.getElementById('filenameForm').attachEvent("submit", processForm);
} else {
	document.getElementById('filenameForm').addEventListener("submit", processForm);
}
...
function setNodesAndLines(layerId) {
	...
	lines = [];
	nodes = [];
	currentLayer = new Kinetic.Layer();
	currentLayerNum = layerId;
